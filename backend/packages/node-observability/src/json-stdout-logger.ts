...
export interface ActiveTraceContext { readonly spanId: string; readonly traceId: string }
...
function limitValue(value: LogAttributeValue, depth = 0): LogAttributeValue {
  ...
  if (Array.isArray(value)) {
    return value.slice(0, MAX_ENTRIES).map((item) => limitValue(item, depth + 1));
  }
  return Object.fromEntries(Object.entries(value).slice(0, MAX_ENTRIES)
    .map(([key, item]) => [key, limitValue(item, depth + 1)]));
}

function stdoutWriter(line: string): void { void process.stdout.write(line); }
...
export class JsonStdoutLogger implements Logger {
  ...
  log(record: LogRecord): void {
    ...
    try {
      const attributes = Object.fromEntries(Object.entries(record.attributes)
