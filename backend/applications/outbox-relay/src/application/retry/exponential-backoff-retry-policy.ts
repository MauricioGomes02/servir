...
import type { Clock, RandomSource, RetryDecision, RetryPolicy } from '@/application/ports';
...
export class ExponentialBackoffRetryPolicy implements RetryPolicy {
  ...
  decide(
    ...
    if (!Number.isFinite(randomValue) || randomValue < 0 || randomValue >= 1) {
      throw new RetryPolicyConfigError(RetryPolicyConfigErrorCodes.InvalidRandomValue);
    }

    const exponentialDelay =
      this.config.baseDelayMilliseconds * 2 ** Math.max(0, input.attemptCount - 1);
    const boundedDelay = Math.min(exponentialDelay, this.config.maximumDelayMilliseconds);
    const jitterFactor = 1 - this.config.jitterRatio + 2 * this.config.jitterRatio * randomValue;
...
function validateConfig(config: ExponentialBackoffRetryPolicyConfig): void {
  if (!Number.isInteger(config.maximumAttempts) || config.maximumAttempts <= 0) {
    throw new RetryPolicyConfigError(RetryPolicyConfigErrorCodes.InvalidMaximumAttempts);
  }

  if (!Number.isInteger(config.baseDelayMilliseconds) || config.baseDelayMilliseconds <= 0) {
    throw new RetryPolicyConfigError(RetryPolicyConfigErrorCodes.InvalidBaseDelay);
  }
  ...
  if (
    ...
    throw new RetryPolicyConfigError(RetryPolicyConfigErrorCodes.InvalidMaximumDelay);
  }

  if (!Number.isFinite(config.jitterRatio) || config.jitterRatio < 0 || config.jitterRatio > 1) {
    throw new RetryPolicyConfigError(RetryPolicyConfigErrorCodes.InvalidJitterRatio);
  }
}
