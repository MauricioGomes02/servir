...
describe('PostgreSQL ministry persistence', () => {
  integrationIt('commits ministry and outbox atomically and rolls both back on outbox failure', async (testContext) => {
    ...
    await cleanup();
    await inspection.query('INSERT INTO organizations (id, name) VALUES ($1, $2)', [ORGANIZATION_ID, 'Ministry integration organization']);
    ...
    const context = createExecutionContext({ correlationId: value(parseCorrelationId('ministry-integration')) });
    const common = {
    ...
    const committed = new CreateMinistryHandler({
      ...
      domainEventIdGenerator: new SequenceIdGenerator<DomainEventId>([value(parseDomainEventId(EVENT_ID))]),
      messageIdGenerator: new SequenceIdGenerator<MessageId>([value(parseMessageId(MESSAGE_ID))]),
    });
    const result = await committed.handle({ organizationId: organizationId.toString(), name: 'Louvor' }, context);
    assert.equal(result.success, true);
    ...
    const failing = new CreateMinistryHandler({
      ...common,
      ministryIdGenerator: new SequenceIdGenerator([value(MinistryId.create(ROLLED_BACK_MINISTRY_ID))]),
      domainEventIdGenerator: new SequenceIdGenerator<DomainEventId>([value(parseDomainEventId(ROLLED_BACK_EVENT_ID))]),
      messageIdGenerator: new SequenceIdGenerator<MessageId>([value(parseMessageId(MESSAGE_ID))]),
    });
    ...
    assert.equal((await inspection.query('SELECT 1 FROM ministries WHERE id = $1', [ROLLED_BACK_MINISTRY_ID])).rowCount, 0);
  });
});
