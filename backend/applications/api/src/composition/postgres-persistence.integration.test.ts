...
describe('PostgreSQL organization persistence', () => {
  integrationIt('commits organization and outbox atomically and rolls both back on failure', async (testContext) => {
    ...
    const rolledBackEventId = requireValue(
      parseDomainEventId(ROLLED_BACK_EVENT_ID),
    );
    const messageId = requireValue(parseMessageId(MESSAGE_ID));

    async function cleanup(): Promise<void> {
      await inspectionPool.query(
        'DELETE FROM outbox_messages WHERE message_id = $1',
        [MESSAGE_ID],
      );
      await inspectionPool.query(
    ...
    await assert.rejects(
      ...
      (error: unknown) => error instanceof PostgresEventOutboxError
        && error.code === PostgresEventOutboxErrorCode,
    );
