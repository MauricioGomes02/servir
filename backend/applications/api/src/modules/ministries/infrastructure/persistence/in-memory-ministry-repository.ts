...
export class InMemoryMinistryRepository implements MinistryRepository {
  ...
  async add(ministry: Ministry) {
    const conflict = this.storedMinistries.some(
      (candidate) =>
  ...
  async findById(organizationId: OrganizationId, ministryId: MinistryId) {
    const found = this.storedMinistries.find(
      (candidate) =>
  ...
  get ministries(): ReadonlyArray<Ministry> {
    return Object.freeze([...this.storedMinistries]);
  }
}
