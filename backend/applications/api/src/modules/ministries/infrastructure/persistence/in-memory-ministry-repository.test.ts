...
describe('InMemoryMinistryRepository', () => {
  it('rejects a second active name ignoring case without changing stored state', async () => {
    ...
    assert.equal(
      (await repository.add(ministry('0198f334-6dc5-7c20-9af1-91d7e599e031', 'Louvor'))).success,
      true,
    );
    const duplicate = await repository.add(
      ministry('0198f334-6dc5-7c20-9af1-91d7e599e032', 'louvor'),
    );
  ...
  it('preserves accents when comparing active names', async () => {
    ...
    assert.equal(
      (await repository.add(ministry('0198f334-6dc5-7c20-9af1-91d7e599e034', 'Midia'))).success,
      true,
    );
