...
describe('MinistryCreationPolicy', () => {
  ...
  it('allows a new active name in an existing organization', () => {
    assert.deepEqual(policy.evaluate({ organizationExists: true, activeNameExists: false }), { success: true, value: undefined });
  });
  ...
  it('rejects an active duplicate name', () => {
    ...
    assert.equal(result.success, false);
    if (!result.success) assert.equal(result.error.code, MinistryCreationPolicyErrorCodes.ActiveNameAlreadyExists);
  });
});
