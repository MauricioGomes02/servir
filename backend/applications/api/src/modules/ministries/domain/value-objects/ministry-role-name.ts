...
export class MinistryRoleName extends ValueObject<Props, 'MinistryRoleName'> {
  ...
  static create(input: unknown): Result<MinistryRoleName, MinistryRoleNameError> {
    if (typeof input !== 'string') return failure({ code: MinistryRoleNameErrorCodes.InvalidType, field: 'name' });
    const value = input.trim();
    if (value.length === 0) return failure({ code: MinistryRoleNameErrorCodes.Empty, field: 'name' });
    if (value.length > MAX_LENGTH) return failure({ code: MinistryRoleNameErrorCodes.TooLong, field: 'name', params: { maxLength: MAX_LENGTH, actualLength: value.length } });
