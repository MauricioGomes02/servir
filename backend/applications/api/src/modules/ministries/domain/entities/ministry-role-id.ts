...
export class MinistryRoleId extends EntityId<'MinistryRoleId'> {
  ...
  static create(input: unknown): Result<MinistryRoleId, MinistryRoleIdError> {
    if (typeof input !== 'string')
      return failure({ code: MinistryRoleIdErrorCodes.InvalidType, field: 'ministryRoleId' });
    const value = input.trim();
    if (value.length === 0)
      return failure({ code: MinistryRoleIdErrorCodes.Empty, field: 'ministryRoleId' });
    if (value.length > MAX_LENGTH)
      return failure({
        code: MinistryRoleIdErrorCodes.TooLong,
        field: 'ministryRoleId',
        params: { maxLength: MAX_LENGTH, actualLength: value.length },
      });
    if (!isCanonicalUuid(value))
      return failure({ code: MinistryRoleIdErrorCodes.InvalidFormat, field: 'ministryRoleId' });
