...
> extends Entity<TId, TProps> {
  ...
  protected recordDomainEvent(event: TDomainEvent): void {
  ...
  acknowledgeDomainEvents(events: ReadonlyArray<TDomainEvent>): void {
    const matchesPendingSequence = events.every(
      (event, index) => this.domainEvents[index]?.eventId === event.eventId,
    );

    if (!matchesPendingSequence) {
      throw new DomainEventAcknowledgementError(this.domainEvents.length, events.length);
    }
