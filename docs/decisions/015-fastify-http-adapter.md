...
W3C Trace Context permanece responsabilidade da futura instrumentação OpenTelemetry. `traceId`, `requestId` e `correlationId` mantêm semânticas distintas; baggage fica desabilitado sem allowlist explícita.
...
A aplicação precisa manter políticas explícitas para códigos HTTP, validação estrutural, negociação de locale, logs e respostas seguras. OpenTelemetry, DI automática e adapters HTTP de casos de uso permanecem incrementos separados.
