...
| Aplicação de relay durável | Implementação inicial | Storage PostgreSQL, retry com jitter, publisher Kafka/CloudEvents, propagação W3C, configuração, ciclo contínuo cancelável, logs estruturados e encerramento seguro estão implementados; métricas, operação de reprocessamento e testes reais com Kafka permanecem planejados |
