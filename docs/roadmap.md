...
| Aplicação de relay durável | Implementação inicial | Comportamento de lote, storage PostgreSQL, retry com jitter, publisher Kafka/CloudEvents e propagação W3C estão testados; composição executável, ciclo contínuo, logs/métricas e encerramento seguro permanecem planejados |
