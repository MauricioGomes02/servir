...
| Participação e qualificação ministerial | Corte vertical inicial | RequestMinistryMembership implementado como Aggregate Root separado, com Policy sobre fatos mínimos, unicidade de vínculo vigente, persistência/outbox atômicas, Integration Event v1 e entrada HTTP; aprovação, estados posteriores e qualificação permanecem planejados |
...
| Adapter HTTP e Composition Root | Implementação inicial | Factory Fastify, container Awilix estrito e tipado restrito à composição, registros por módulo, contexto explícito por request, protocolo compartilhado de Problem Details, representação direta do recurso, log final e spans semânticos estão testados; HTTP/Fastify/PostgreSQL permanecem instrumentados automaticamente |
