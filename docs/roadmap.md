...
| Logger | Implementação inicial | Contrato em `application-foundation` e adapter JSON em `node-observability` são compartilhados por API e relay; contexto, imutabilidade, filtro por `LOG_LEVEL`, limites, correlação e narrativas de negócio estão testados; redaction configurável permanece planejada |
