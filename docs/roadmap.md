...
| Adapter HTTP e Composition Root | Implementação inicial | Factory Fastify, container Awilix tipado restrito à composição, Service Collection, Mediator tipado, manifestos instaláveis por módulo, persistência única PostgreSQL-only, contexto explícito por request, Problem Details, log final e spans semânticos estão testados; doubles em memória pertencem somente ao test harness |
