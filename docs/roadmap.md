...
| Times ministeriais | Cortes verticais iniciais | CreateMinistryTeam e AssignMemberToTeam implementados como roots históricas separadas, com isolamento por Organization, persistência/outbox atômicas, Integration Events v1 e entradas HTTP; liderança vigente e responsabilidade por escala permanecem planejadas |
...
| Infraestrutura local e migrations | Implementação inicial | Stacks Terraform, Liquibase e fluxo PostgreSQL/outbox/Kafka foram validados; tabelas tenant-owned carregam OrganizationId e constraints compostas impedem referências cruzadas; RLS, credenciais de runtime e IaC de ambientes compartilhados permanecem planejados |
