...
A interface reside em um workspace independente com duas aplicações. A web Vue executa no navegador; seus limites técnicos separam `app`, pages, features, entities de frontend e código shared sem reproduzir as camadas do backend por simetria. Componentes não conhecem `fetch`, caminhos do BFF ou contratos da API interna. A URL preserva o `organizationId` como contexto explícito do tenant, enquanto estado global só entra quando houver compartilhamento e lifecycle próprios. A estrutura e a migração incremental estão no guia de [Arquitetura do frontend web](frontend-architecture.md) e no [ADR 064](decisions/064-feature-oriented-frontend-architecture.md).

A arquitetura de informação representa o trabalho do usuário, não Aggregates ou endpoints. Páginas respondem a perguntas; features conduzem tarefas; estados percebidos como carregando, vazio, erro, sucesso, rascunho e publicado integram o contrato da experiência. A linguagem visual usa tokens semânticos nos temas claro e escuro, e componentes só são promovidos ao escopo compartilhado quando consumidores reais comprovam o mesmo comportamento. O guia está em [Experiência da aplicação web](frontend-experience.md), com decisões registradas nos [ADRs 059](decisions/059-product-visual-language-and-reusable-web-components.md) e [060](decisions/060-task-oriented-frontend-experience.md).

O BFF Fastify é a única fronteira pública de servidor da interface. O navegador acessa somente rotas relativas `/bff/*`; cada rota é registrada explicitamente e traduz uma necessidade da experiência para a API privada configurada por `API_BASE_URL`. Quando uma tarefa exigir composição, o contrato público do BFF pode diferir dos endpoints internos, mas não incorpora regras de domínio, autorização ou isolamento do tenant. O BFF também serve o bundle estático da web, portanto web e BFF possuem código, build e testes separados, mas o artefato inicial executa apenas o processo do BFF. Uma rede de aplicação conecta exclusivamente BFF e API; somente o BFF participa da borda e publica HTTP. No ambiente local, uma porta opcional em loopback permite executar o BFF no host com hot reload sem tornar a API acessível em interfaces externas. A API não se torna confiável por estar numa rede privada ou em loopback: autenticação, autorização e isolamento de Organization continuam responsabilidades de suas fronteiras. As decisões completas estão nos [ADRs 057](decisions/057-vue-web-application-foundation.md), [058](decisions/058-private-api-behind-containerized-frontend-bff.md), [060](decisions/060-task-oriented-frontend-experience.md) e [061](decisions/061-optional-loopback-api-for-local-development.md).
...
Identity & Access separa `User` global, `OrganizationAccess` tenant-scoped e `MemberAccessInvitation`. O BFF integra diretamente Google e Microsoft por OIDC e emite credenciais próprias com audiences separadas para sessão e API; tokens externos não autorizam operações internas. Autorização continua na API, `organizationId` apenas seleciona contexto e vínculos nunca são inferidos por nome ou e-mail. A estratégia combina relação tenant-scoped, roles/capabilities e policies contextuais de forma incremental, conforme o guia de [Autorização](authorization.md). As decisões estão nos [ADRs 066](decisions/066-identity-access-and-member-linking.md), [067](decisions/067-direct-oidc-and-servir-issued-credentials.md) e [070](decisions/070-incremental-hybrid-authorization.md).

O contexto causal distingue actor, executor e source. Actor preserva quem iniciou a operação; executor identifica o serviço da etapa atual; source descreve opcionalmente a origem inicial confiável. Logs e traces continuam de melhor esforço e operacionais, enquanto auditoria durável registra apenas ações relevantes e usa outbox quando precisa ser atômica com uma mudança. IP e user agent são dados opcionais e minimizados, nunca identidades. A decisão está no [ADR 068](decisions/068-causal-execution-context-and-audit-boundary.md).
