...
Há implementações iniciais das primitivas centrais de domínio, mensagens, contexto, logging, localização de erros, tempo, identidade e Unit of Work. O primeiro corte vertical possui composição executável, rota HTTP, log estruturado final, tracing semântico do caso de uso e persistência atômica de Organization + outbox em memória ou PostgreSQL. No modo PostgreSQL, `OrganizationCreated` é traduzido explicitamente para um Integration Event v1 antes da persistência. A aplicação `outbox-relay` já possui processamento de lote, storage PostgreSQL, retry exponencial com jitter e publisher Kafka com CloudEvents e W3C Trace Context; o processo contínuo permanece planejado. O contrato serializável compartilhado limita-se a Integration Events e valores JSON. PostgreSQL local, Liquibase e o schema inicial são externos às aplicações. Repository, Specification e Policy permanecem orientados pelos primeiros consumidores concretos.
