...
export class InternalCredentialIssuer {
  ...
  static async create(
    ...
    if (!(signingKey instanceof CryptoKey)) {
      throw new BffAuthenticationError(BffAuthenticationErrorCodes.SigningKeyInvalid);
    }
    ...
    if (!(verificationKey instanceof CryptoKey)) {
      throw new BffAuthenticationError(BffAuthenticationErrorCodes.VerificationKeyInvalid);
    }
  ...
  async verifySessionToken(
    ...
    let result;
    try {
      result = await jwtVerify(token, this.verificationKey, {
      ...
      throw new BffAuthenticationError(BffAuthenticationErrorCodes.SessionInvalid, {
        cause: error,
      });
    }
    if (
